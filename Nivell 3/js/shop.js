...
function updateCartNumber() {
    ...
    countProduct.innerText = number;
    let hidden = document.querySelectorAll('.cart-hidden');
...
function applyPromotionsCart() {
    ...
    for (const item of cart) {
        if ((item.id === 1  || item.id === 3) && item.quantity >= item.product.offer.number) {
...
function printCart() {
    ...
    for (const item of cart) {
        ...
        itemHTML += `<td class="add-button"><button class="btn btn-primary" onclick="buyFromCart(${item.id})"><i class="fa-solid fa-plus"></i></button></td>`;
        itemHTML += `<td class="trash-button"><button class="btn btn-dark" onclick="removeFromCart(${item.id})"><i class="fa-solid fa-trash"></i></button></td>`
...
function managePromotions(itemIndex) {
    var item = cart[itemIndex];
    if ((item.id === 1 || item.id === 3) && item.subtotalWithDiscount != null) {
        if (item.product.offer.number > item.quantity) {
